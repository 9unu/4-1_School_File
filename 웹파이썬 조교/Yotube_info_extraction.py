...
def video_info_extract(youtube, YouTube_video, start_idx, end_idx):
    ...
    for i in range(len(YouTube_video)):
        request = youtube.videos().list(
            ...
            part='snippet, contentDetails, statistics, topicDetails'  # fileDetails 제거
        )
        ...
        if not response['items']:
            ...
            watched_time.append(YouTube_video.iloc[i]['time'])  # 원본 데이터프레임의 시간을 추가
            headers.append(YouTube_video.iloc[i]['header'])  # 원본 데이터프레임의 헤더(yotube 영상인지 or 음악인지)

            # 추가된 정보
            descriptions.append(item['snippet']['description'])
            thumbnails.append(item['snippet']['thumbnails']['default']['url'])  # 기본 썸네일 URL
